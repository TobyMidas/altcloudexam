...
sudo mysql -uroot -e "CREATE DATABASE Morris;"
sudo mysql -uroot -e "CREATE USER 'ugo'@'localhost' IDENTIFIED BY 'ada';"
sudo mysql -uroot -e "GRANT ALL PRIVILEGES ON Morris.* TO 'ugo'@'localhost';"
cd /var/www/laravel
...
sudo sed -i '25 s/=laravel/=Morris/' /var/www/laravel/.env
sudo sed -i '26 s/=root/=ugo/' /var/www/laravel/.env
sudo sed -i '27 s/=/=ada/' /var/www/laravel/.env
